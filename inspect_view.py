...
def filter_json_lines(lines, json_data, filter_string, case_sensitive=False):
    """Filter JSON lines based on filter expression"""
    if not filter_string:
    ...
    # For JSON filtering, we can be smarter and filter by key-value pairs
    flattened = flatten_json(json_data)
    
    # Create a mapping of line numbers to their content including keys
    line_to_content = {}
    for i, line in enumerate(lines):
        # For each line, find which keys/values it contains
        line_content = line
        for key, value in flattened:
            if value in line or key in line:
                line_content += f" {key}={value}"
        line_to_content[i] = line_content
    ...
    for i, line in enumerate(lines):
        # Check both the line itself and its associated key-value pairs
        if evaluate_filter(tokens, line_to_content.get(i, line), case_sensitive):
    ...
    try:
        ...
        while running:
            ...
            if search_mode:
                ...
                # Show filter help
                help_text = "Syntax: term AND/OR term, +include -exclude \"multi word\""
